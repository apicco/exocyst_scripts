...
get.contour<-function(image.data,input.range=c(0,0.01)){
	...
	seg=opening(seg,mk)
	seg=fillHull(seg)
	#ORIGINAL seg=bwlabel(seg)
	
	seg.erode<-erode(seg,mk2)
	contour<-seg-seg.erode
	contour<-bwlabel(contour)#NEW
...
load.data.collection<-function(directory,image.directory=c()){
	...
	for (i in 1:length(W2)){
		...
		if (!is.null(image.directory)) {
			...
			image.contour<-get.contour( (image.data[,,1] + image.data[,,2]) / 2 )
			#MAX PROJECTION image.contour<-get.contour( apply(image.data,c(1,2),max) )
			#ORIGINAL image.contour<-get.contour( image.data[,,2] )
